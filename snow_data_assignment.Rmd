...
# #overkill
# empty<-list()
# name_data<-for (i in 1:length(dataset)){
...
#     name = str_split_fixed(file_names,'/',2)[,2]%>%
#     gsub('_Forcing_Data.txt','',.)%>%
#   empty[[1]]<-read_delim(file_names[i], col_names=headers,show_col_types = FALSE)%>%
#     select(c(1:20))%>%
#     mutate(site = name[i]) 
#   return(empty)
#   }}
...
ggplot(temperature, aes(x=year, y=meanT, color=site))+
  geom_line()+
  ggthemes::theme_few()
message("Site SBB_SASP has a higher yearly mean temperature than site SBB_SBSP for all years in the dataset.")
```
...
loopy<-function(data){
  for (i in 1:length(temperature)){
    df<-data%>%
      group_by(month, site)%>%
      summarize(meanT=mean(`air temp [K]`))
      print(ggplot(df, aes(x=month, y=meanT, color=site))+
            geom_line())
...
ggplot(precip, aes(x=year, y=meanPre))+
  geom_point()+
  ggthemes::theme_few()
```
